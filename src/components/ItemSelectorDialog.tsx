...
// --- FUNÇÃO DE BUSCA CORRIGIDA (REMOÇÃO DA CONVERSÃO DE CASO) ---
const fetchItemsForSelector = async (tableId: string, categories: string[]) => {
	// 1. Não formatamos mais as categorias.
	// Usamos as categorias diretamente (assumindo que são minúsculas: 'weapon', 'armor', etc.)
	const targetCategories = categories.map(c => c.toLowerCase());

	let query = supabase
		.from("items")
		// Seleção minimalista e segura de colunas
		.select("id, name, description, type, data") 
		
		// CORREÇÃO CRÍTICA (Mantida): Busca itens da mesa ATUAL (table_id.eq.X) OU itens GLOBAIS (table_id.is.null)
		.or(`table_id.eq.${tableId},table_id.is.null`) 
		.order("name");
		
	if (targetCategories.length > 0) {
		// CORREÇÃO DE CASO: O filtro .in() usa as categorias minúsculas originais.
