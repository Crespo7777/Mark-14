...
import { 
  Book, Calendar, MoreVertical, ImageOff, Tag,
  Edit, Trash2, Eye
} from "lucide-react";
...
// Adapte esta interface se necessário, baseada na sua estrutura real
...
interface JournalEntryCardProps {
  ...
  isGM?: boolean; // Para saber se mostra opções de mestre
}
...
export function JournalEntryCard({
  ...
  const dateLabel = formatDistanceToNow(new Date(entry.created_at), { addSuffix: true, locale: ptBR });
  ...
  return (
    <Card 
      className={cn(
        ...
        "h-[180px]" // Altura fixa para uniformidade
      )}
    ...
    >
      ...
      <div className="relative h-24 w-full shrink-0 overflow-hidden bg-muted">
        ...
        {/* Menu de Ações (Só aparece se tiver onEdit ou isGM) */}
        {(isGM || onEdit) && (
