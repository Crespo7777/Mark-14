...
interface CharacterShareDialogProps {
...
export const CharacterShareDialog = ({ character, open, onOpenChange, onSaved }: CharacterShareDialogProps) => {
  ...
  useEffect(() => {
    if (open && character) {
      setSelectedPlayers(character.shared_with_players || []);
    }
  }, [open, character]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({ title: "Visibilidade Atualizada", description: `Permissões de ${character.name} alteradas.` });
      onSaved();
      onOpenChange(false);
    } catch (error: any) {
      console.error(error);
      ...
      <DialogContent className="sm:max-w-[425px] bg-background border-border shadow-2xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Partilhar Personagem
          </DialogTitle>
          <DialogDescription>
            Defina quem pode ver/editar a ficha de <strong>{character.name}</strong>.
          </DialogDescription>
        </DialogHeader>
        ...
        {players.length > 0 ? (
          <>
            <div className="flex items-center space-x-3 p-3 rounded-lg bg-muted/40 border border-border/40 hover:bg-muted/60 transition-colors">
              ...
              />
              <Label htmlFor="share-all-char" className="flex-1 cursor-pointer font-bold select-none text-sm">
                Todos os Jogadores
              </Label>
            </div>
              ...
              <div className="space-y-1">
                {players.map(player => (
                  <div key={player.id} className="flex items-center space-x-3 p-2 rounded-md hover:bg-accent/50 transition-colors">
                    <Checkbox
                      id={`share-char-${player.id}`}
                      checked={selectedPlayers.includes(player.id)}
                      onCheckedChange={(c) => handleCheckedChange(player.id, c as boolean)}
                    />
                    <Label htmlFor={`share-char-${player.id}`} className="flex-1 cursor-pointer flex items-center gap-3 select-none py-1">
