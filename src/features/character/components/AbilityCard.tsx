...
export const AbilityCard = ({ index, remove, isReadOnly, handleRoll }: AbilityCardProps) => {
    const { control, watch, setValue } = useFormContext();
    const [isExpanded, setIsExpanded] = useState(false);
    
    const name = watch(`abilities.${index}.name`) || "Nova Habilidade";
    const isActive = watch(`abilities.${index}.isActive`);
    const level = watch(`abilities.${index}.level`) || "Novato";
    const type = watch(`abilities.${index}.type`);
    const attrKey = watch(`abilities.${index}.associatedAttribute`);
    const corruption = watch(`abilities.${index}.corruptionCost`);
    ...
    const handleMainAction = () => {
        if (hasAttr) {
            // Se tem atributo, ROLA OS DADOS
            handleRoll(index);
        } else {
            // Se não tem atributo (ex: Amoque), ALTERNA O ESTADO (Ativar/Desativar)
    ...
    return (
        <Card className={cn("transition-all duration-300 shadow-sm overflow-hidden border-l-4", levelStyles[level] || "border-border", isExpanded ? "ring-1 ring-primary/20" : "")}>
                ...
                <div className="flex items-start gap-3 flex-1 min-w-0">
                    ...
                    <div className="flex flex-col min-w-0 w-full">
                        <div className="flex items-center justify-between w-full">
                            ...
                            {/* Switch Visual (Sincronizado com o botão principal) */}
                            <FormField control={control} name={`abilities.${index}.isActive`} render={({field}) => (
                                <Switch 
                                    ...
                                    title={field.value ? "Desativar Habilidade" : "Ativar Habilidade"}
                                />
                            )}/>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-1 mt-1 items-center">
                            ...
                            {isActive && (
                                <Badge variant="default" className="text-[9px] h-4 px-1.5 animate-pulse bg-green-600 hover:bg-green-600 border-none">
                                    ATIVO
                                </Badge>
                            )}
                        </div>
            ...
            <div className="px-3 pb-3 flex gap-2">
                <Button 
                    size="sm" 
                    // Muda a cor e o ícone dependendo da ação
