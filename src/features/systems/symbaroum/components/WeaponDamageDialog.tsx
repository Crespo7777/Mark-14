...
// Imports do Sistema
import { formatDamageRoll } from "../utils/symbaroum-dice"; 
import { buildDamagePayload } from "../utils/symbaroum-discord";
...
export const WeaponDamageDialog = ({
  ...
  const handleRoll = async (isHidden: boolean) => {
    ...
    // Formatação Discord
    const discordPayload = buildDamagePayload(characterName, weaponName, totalDamage, baseRoll, advantageRoll, modValue);

    // Envio para BD e Discord
    if (isHidden && isMaster) {
