...
// Imports do Sistema
import { formatAttackRoll, type AttributeRollResult } from "../utils/symbaroum-dice";
import { buildAttackPayload } from "../utils/symbaroum-discord";
...
export const WeaponAttackDialog = ({
  ...
  const handleRoll = async (isHidden: boolean) => {
    ...
    setLoading(true);
    if (onConfirm) onConfirm(); // Consome munição, etc.
    ...
    if (tableId) {
        ...
        if (user) {
            ...
            // Monta objeto de resultado padronizado
            const resultData: AttributeRollResult = {
            ...
            const chatMessage = formatAttackRoll(characterName, weaponName, attributeName, resultData);

            // Formata payload JSON para Discord
            const discordPayload = buildAttackPayload(characterName, weaponName, attributeName, resultData);
            ...
            if (isHidden && isMaster) {
                ...
                supabase.functions.invoke('discord-roll-handler', {
                    body: { tableId, discordPayload, userName: characterName }
                }).catch(() => {});
            }
        }
    }
