...
export const AttributesTab = () => {
  ...
  const remainingAttributePoints = calculations?.remainingAttributePoints ?? 0;
    ...
    <div className="h-full flex flex-col space-y-6 p-2 md:p-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 md:gap-6">
        {attributesList.map((attr) => {
          ...
          // --- CORREÇÃO DO CRASH ---
          // Verifica se rawVal não é nulo antes de acessar propriedades
          const currentValue = (rawVal !== null && typeof rawVal === 'object') 
            ? Number(rawVal.value) 
            : Number(rawVal) || 0;
                    ...
                    <FormControl>
                      <div className="relative">
                          <Input 
                            ...
                            {...field}
                            // Tratamento seguro no value e onChange
                            value={field.value !== null && typeof field.value === 'object' ? field.value.value : (field.value ?? 0)}
                            onChange={e => {
                               const val = e.target.valueAsNumber;
                               if (field.value !== null && typeof field.value === 'object') {
                                   field.onChange({ ...field.value, value: val });
                               } else {
                                   field.onChange(val);
                               }
                            }}
