...
interface NpcShareDialogProps {
  npc: NpcWithRelations | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSaved: () => void;
}

export const NpcShareDialog = ({ npc, open, onOpenChange, onSaved }: NpcShareDialogProps) => {
  const [selectedPlayers, setSelectedPlayers] = useState<string[]>([]);
  const [loading, setLoading] = useState(false);
  const { members } = useTableContext();
  const { toast } = useToast();

  // Filtra apenas jogadores (remove o Mestre da lista)
  ...
  // Sincroniza o estado quando o diálogo abre
  useEffect(() => {
    if (open && npc) {
      setSelectedPlayers(npc.shared_with_players || []);
    }
  }, [open, npc]);
  ...
  const handleSave = async () => {
    ...
    try {
      // Lógica de Negócio: Se todos estiverem selecionados, consideramos "Partilha Global" (is_shared = true)
      // Caso contrário, é uma partilha seletiva.
      ...
      const { error } = await supabase
        ...
        .update({ 
            ...
            is_shared: isSharedGlobal 
        })
        .eq("id", npc.id);

      if (error) throw error;

      toast({ title: "Visibilidade Atualizada", description: `Permissões de ${npc.name} alteradas.` });
      onSaved();
      onOpenChange(false);
    } catch (error: any) {
      console.error(error);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px] bg-background border-border shadow-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Defina quem pode ver a ficha de <strong>{npc.name}</strong>.
          </DialogDescription>
        </DialogHeader>
        ...
        {players.length > 0 ? (
          <>
            <div className="flex items-center space-x-3 p-3 rounded-lg bg-muted/40 border border-border/40 hover:bg-muted/60 transition-colors">
              ...
              />
              <Label htmlFor="share-all-npc" className="flex-1 cursor-pointer font-bold select-none text-sm">
                Todos os Jogadores
              </Label>
            </div>
              ...
              <div className="space-y-1">
                {players.map(player => (
                  <div key={player.id} className="flex items-center space-x-3 p-2 rounded-md hover:bg-accent/50 transition-colors">
                    ...
                    />
                    <Label htmlFor={`share-${player.id}`} className="flex-1 cursor-pointer flex items-center gap-3 select-none py-1">
