...
// Restaurando exatamente as suas cores originais
...
export const NpcAttributesTab = () => {
  const { npc, isReadOnly } = useSymbaroumNpcSheet();
  ...
  const getModifierStyle = (value: number) => { 
      const mod = 10 - value; 
      if (mod < 0) return "bg-red-100 text-red-700 border-red-200"; 
      if (mod > 0) return "bg-green-100 text-green-700 border-green-200"; 
