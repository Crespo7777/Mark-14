...
import { Plus, Trash2, Save, BookOpen, Search, Crown, Users, Eye, EyeOff } from "lucide-react";
...
interface Rule {
  ...
  title: string;
  content: string;
  is_gm_only: boolean; // Nova coluna
  updated_at: string;
}
...
export const MasterRulesTab = ({ tableId }: { tableId: string }) => {
  ...
  // Filtra primeiro pela aba (Publico vs Mestre) e depois pela busca
  const filteredRules = rules.filter(r => {
      const matchesSearch = r.title.toLowerCase().includes(searchQuery.toLowerCase());
      const matchesTab = activeListTab === "gm" ? r.is_gm_only : !r.is_gm_only;
  ...
  const handleCreateNew = () => {
      // Se estiver na aba de Mestre, cria como Mestre por padrão
      const isGm = activeListTab === "gm";
      const newRule = { id: "new", title: "Nova Regra", content: "", is_gm_only: isGm, updated_at: new Date().toISOString() };
          ...
          <div className="p-4 border-b border-border/50 bg-muted/10 space-y-3">
             ...
             <Tabs value={activeListTab} onValueChange={setActiveListTab} className="w-full">
                <TabsList className="w-full grid grid-cols-2 bg-background/50">
                    ...
                    <TabsTrigger value="gm" className="text-xs data-[state=active]:text-amber-500">
                ...
                <div className="flex flex-col p-2 gap-1">
                    ...
                    {filteredRules.length === 0 && !isLoading && (
                        <div className="flex flex-col items-center justify-center py-10 text-muted-foreground opacity-50">
                            {activeListTab === "gm" ? <Crown className="w-8 h-8 mb-2" /> : <BookOpen className="w-8 h-8 mb-2" />}
                            <p className="text-xs">Nenhuma regra nesta categoria.</p>
                        </div>
                    )}
                </div>
             </ScrollArea>
