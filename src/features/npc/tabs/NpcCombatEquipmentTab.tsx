...
export const NpcCombatEquipmentTab = () => {
  ...
  const handleDamage = (rawDamage: number) => {
    ...
    if (actualDamage > painThreshold && painThreshold > 0) {
      toast({
        ...
        description: "O dano superou o Limiar de Dor. O NPC pode estar atordoado.",
        variant: "destructive",
      });
    }
  };
  ...
  const handleAttackClick = (index: number) => {
    ...
    setAttackRollData({
      abilityName: weapon.name || "Ataque NPC",
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Heart className="text-red-500" /> Vitalidade & Resistência
            </CardTitle>
          </CardHeader>
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <FormField
                    ...
                    <FormItem>
                        ...
                        <FormControl><Input type="number" className="text-2xl font-bold h-12 bg-muted/50" {...field} readOnly /></FormControl>
                    </FormItem>
                    )}
                />
            </div>
                  ...
                  <FormControl>
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        else if (/^-?\d*$/.test(val)) { setDisplayDefense(val); field.onChange(parseInt(val, 10)); }
                      }}
                      readOnly={isReadOnly}
                    />
                  </FormControl>
                    ...
                    <AccordionContent className="pt-4 mt-3 border-t border-border/50">
                       <div className="space-y-4">
                          ...
                          <FormField control={form.control} name={`weapons.${index}.quality`} render={({field}) => (
                             <FormItem><FormLabel>Qualidades</FormLabel><FormControl><Input placeholder="Veneno" {...field} readOnly={isReadOnly}/></FormControl></FormItem>
                          )}/>
                       </div>
                    </AccordionContent>
                 </AccordionItem>
                       ...
                       <div className="space-y-4">
                          ...
                          <div className="grid grid-cols-2 gap-4">
                              ...
                              <FormField control={form.control} name={`armors.${index}.quality`} render={({field}) => (
                                 <FormItem><FormLabel>Qualidades</FormLabel><FormControl><Input placeholder="Natural" {...field} readOnly={isReadOnly}/></FormControl></FormItem>
                              )}/>
                          </div>
                       </div>
                    </AccordionContent>
