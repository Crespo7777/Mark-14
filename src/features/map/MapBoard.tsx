...
import { MapToken as MapTokenType, FogShape } from "@/types/map-types";
...
const BackgroundImage = React.memo(({ url, width, height }: { url: string | null, width: number, height: number }) => {
    const [image] = useImage(url || "", "anonymous");
    if (!url || !image) return <Rect width={width} height={height} fill="#18181b" listening={false} />;
...
export const MapBoard = ({ width = window.innerWidth, height = window.innerHeight, isMaster = false, tableData }: MapBoardProps) => {
  ...
  const handleDragStart = (e: React.MouseEvent) => {
      ...
      dragOffset.current = { x: e.clientX - combatPos.x, y: e.clientY - combatPos.y };
  };

  useEffect(() => {
      const handleMouseMove = (e: MouseEvent) => {
          if (isDraggingCombat.current) {
              setCombatPos({ x: e.clientX - dragOffset.current.x, y: e.clientY - dragOffset.current.y });
          }
      };
      const handleMouseUp = () => { isDraggingCombat.current = false; };
  ...
  const sendPing = async (x: number, y: number) => {
    const newPing: PingData = { id: crypto.randomUUID(), x, y, color: isMaster ? "#FFD700" : "#ffffff", user: userId };
    addPing(newPing);
    await supabase.channel(`room:${tableId}`).send({ type: 'broadcast', event: 'cursor-ping', payload: newPing });
  };
  ...
  useEffect(() => {
      if (!isFogEnabled && activeTool.startsWith("reveal")) setActiveTool("select");
  ...
  const handleCreateFog = async (points: number[]) => {
      ...
      setFogShapes(prev => [...prev, newShape]);
      
      const { error } = await supabase.from("map_fog").insert({ 
          table_id: tableId, points, type: "reveal", id: newShape.id 
      });
      
      if (error) {
          console.error("Erro fog:", error);
          setFogShapes(prev => prev.filter(s => s.id !== newShape.id));
      }
  };
  ...
  const { interactionHandlers, interactionState, actions } = useMapInteractions({
      ...
      callbacks: {
          ...
          onToolReset: () => setActiveTool("select") // Callback para ESC
      }
  });
  ...
  return (
    ...
    >
      ...
      {isCombatOpen && (
          <div className="absolute z-40 w-72 flex flex-col pointer-events-auto bg-card border border-border/50 rounded-md shadow-2xl overflow-hidden" style={{ left: combatPos.x, top: combatPos.y, maxHeight: 'calc(100vh - 120px)' }}>
              <div className="bg-muted/90 p-1.5 flex items-center justify-between cursor-move select-none border-b" onMouseDown={handleDragStart}>
                  <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-muted-foreground"><GripHorizontal className="w-3 h-3" />Combate</div>
                  <button onClick={() => setIsCombatOpen(false)} className="text-muted-foreground hover:text-foreground"><X className="w-3.5 h-3.5" /></button>
              </div>
      ...
      >
        ...
        <Layer opacity={gridOpacity} listening={false}> 
        ...
        {/* Passamos activeTool para saber como desenhar (fechado ou aberto) */}
        <InteractionLayer state={interactionState} gridSize={gridSize} isMaster={isMaster} activeTool={activeTool} />
        <PingLayer pings={pings} onComplete={removePing} />
      </Stage>

      {tokenHudState && isMaster && (
          <TokenHUD tokenId={tokenHudState.token.id} position={{ x: tokenHudState.x, y: tokenHudState.y }} onClose={() => setTokenHudState(null)} onStartMove={() => actions.startMove(tokenHudState.token)} />
      )}

      {bgMenuState && isMaster && (
          <MapContextMenu position={{ x: bgMenuState.x, y: bgMenuState.y }} tableId={tableId} fogEnabled={isFogEnabled} onClose={() => setBgMenuState(null)} />
      )}
